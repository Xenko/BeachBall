...
BeachBall.DisplayDescription = function(option, status) {
	...
	if (option == 'BorderAlert') {
		...
		$('#ToolFactory').innerHTML = '<a onclick="Molpy.LoadToolFactory(' + status + ')"> <h4>Load Tool Factory</h4> </a> <div id="ToolFactoryDesc"></div>';
...
if (Molpy.Got('Tool Factory') == 1) {
	$('#optionsItems').append('<div class="minifloatbox" id="ToolFactory"> <a onclick="Molpy.LoadToolFactory(' + BeachBall.toolFactory + ')"> <h4>Load Tool Factory</h4> </a> <div id="ToolFactoryDesc"></div></div>');
}
